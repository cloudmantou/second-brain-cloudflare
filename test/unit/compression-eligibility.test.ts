...
describe("compressionEligibilitySql", () => {
  it("includes the importance, recall+age, and both contradiction outcome clauses", () => {
    ...
    expect(sql).toContain("contradiction_losses");
  ...
  it("applies a column prefix to every column when given one", () => {
    ...
    for (const col of ["importance_score", "recall_count", "created_at", "contradiction_wins", "contradiction_losses"]) {
